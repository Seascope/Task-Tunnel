...
- Release hardening adds fixed `<queries>` visibility for Instagram, YouTube, and Reddit, uses Internet permission only for explicit in-app feedback submission, retains no dangerous feedback runtime permissions, updates production service labels/descriptions, and excludes the Attention database/WAL/SHM plus onboarding/Drift preferences from backup/device transfer. `QUERY_ALL_PACKAGES` remains absent.
...
- No post-MVP feature work was started: no additional protected apps, generic discovery/blocking, accounts/cloud sync, telemetry, ML/LLM behavior, downloaded rules, monetization, gamification, browser/VPN control, advanced personalization, or automatic task-completion inference was added. User-initiated feedback delivery is the only new network path.
...
- The local Drift pool uses a small known-app catalog for Instagram, YouTube, TikTok, and Reddit. The production home and debug app shell expose simple local switches backed by `SharedPreferences`. The implementation does not query installed apps, request `QUERY_ALL_PACKAGES`, or expand Task Tunnel surface policy to unsupported apps.
- The soft overlay uses human app labels and the approved observational copy direction. **Set an intention** opens the existing Purpose Gate when the current app is a supported Task Tunnel app, without selecting a task. On Reddit or another unsupported Task Tunnel app it safely acknowledges and dismisses without creating a generic Tunnel.
- Overlay priority is active-Tunnel interaction > Drift check-in > ordinary Purpose Gate. A qualifying Drift check-in consumes the current ordinary Purpose Gate so rapid sequences such as Reddit -> Instagram -> YouTube and Instagram -> YouTube -> TikTok surface Drift immediately. **Keep going** does not reveal the displaced Purpose Gate; **Set an intention** explicitly requests a fresh gate. An active Tunnel, including its M3.1 quick-return grace, still clears/suppresses transient Drift accumulation. A Drift overlay is removed when the foreground becomes a non-selected or internal app.
...
- Historical baseline before the Drift-priority adjustment: 18 focused Drift tests, all 24 M3/M3.1 policy/coordinator regressions, the 81-test debug JVM suite, and `:app:assembleDebug` had passed. The priority adjustment adds two exact-sequence detector regressions; rerun the focused/full Gradle validation and physical M4 matrix after applying it.
